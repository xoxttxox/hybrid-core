...
import { Distribution } from 'hybrid-distribution-types'
...
import { HybridDistribution } from './DistributionFactory'
...
export class DistributionAPI {
    ...
    private distribution!: HybridDistribution
    ...
    public async getDistribution(): Promise<HybridDistribution> {
        if(this.rawDistribution == null) {
            this.rawDistribution = await this.loadDistribution()
            this.distribution = new HybridDistribution(this.rawDistribution, this.commonDir, this.instanceDir)
        }
        return this.distribution
    }

    public async getDistributionLocalLoadOnly(): Promise<HybridDistribution> {
        if(this.rawDistribution == null) {
    ...
    public async refreshDistributionOrFallback(): Promise<HybridDistribution> {
        ...
        if(distro == null) {
            ...
            this.distribution = new HybridDistribution(distro, this.commonDir, this.instanceDir)
