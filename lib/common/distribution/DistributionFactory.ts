import { Distribution, Server, Module, Type, Required as HybridRequired, JavaVersionProps, JavaPlatformOptions, Platform, JdkDistribution } from 'hybrid-distribution-types'
...
export class HybridDistribution {

    private mainServerIndex!: number

    public readonly servers: HybridServer[]
    ...
    constructor(
        ...
        this.servers = this.rawDistribution.servers.map(s => new HybridServer(s, commonDir, instanceDir))
    }
    ...
    public getMainServer(): HybridServer | null {
        return this.mainServerIndex < this.servers.length ? this.servers[this.mainServerIndex] : null
    }

    public getServerById(id: string): HybridServer | null {
...
export class HybridServer {
    ...
    public readonly modules: HybridModule[]
    ...
    constructor(
        ...
        this.modules = rawServer.modules.map(m => new HybridModule(m, rawServer.id, commonDir, instanceDir))
    }
...
export class HybridModule {
    ...
    public readonly subModules: HybridModule[]

    private readonly mavenComponents: Readonly<MavenComponents>
    private readonly required: Readonly<Required<HybridRequired>>
    ...
    constructor(
        ...
        if(this.rawModule.subModules != null) {
            this.subModules = this.rawModule.subModules.map(m => new HybridModule(m, serverId, commonDir, instanceDir))
        } else {
            this.subModules = []
        }
    ...
    private resolveRequired(): Required<HybridRequired> {
    ...
    public getRequired(): Readonly<Required<HybridRequired>> {
